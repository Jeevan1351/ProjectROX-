...
function VerifySec(sec)
{
	...
	else if((sec.charCodeAt(0)<65 || sec.charCodeAt(0)>90)||(sec.charCodeAt(2)<65 || sec.charCodeAt(2)>90))
...
{
	var i;
	if((usn.slice(0,3) == "1BM")&&(usn.charCodeAt(5)<=90 &&usn.charCodeAt(5)>=65)&&(usn.charCodeAt(6)<=90 &&usn.charCodeAt(6)>=65)&&(usn.charCodeAt(7)>=48&&usn.charCodeAt(7)<=57)&&(usn.charCodeAt(8)>=48&&usn.charCodeAt(8)<=57)&&(usn.charCodeAt(9)>=48&&usn.charCodeAt(9)<=57))
	{
...
{
var len = mail.length, mailformat = /^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$/;
 if (mail == "")
{
    document.getElementById("email").innerHTML = "Email Required";
}
else if (mail.match(mailformat) && mail.slice(len - 12, len) == "@bmsce.ac.in") 
{
